...
//
// Chrome sends each message as a 4-byte little-endian length followed by
// that many bytes of UTF-8 JSON, we reply in the same format
// stdout must ONLY ever contain native-messaging frames, tracing goes to stderr
...
// Exits cleanly when stdin is closed - Chrome disconnected or extension port dropped
